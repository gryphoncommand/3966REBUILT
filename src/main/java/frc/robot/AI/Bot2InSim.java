...
public class Bot2InSim extends SubsystemBase {
    ...
    private final Pathfinder botPathfinder = new LocalADStar();
    ...
    private final boolean useLocalCommandRunner = true;
    private boolean activeCommandInitialized = false;

    private boolean isActiveCommandRunning() {
        if (activeCommand == null) return false;
        return useLocalCommandRunner ? activeCommandInitialized : activeCommand.isScheduled();
    }
    ...
    public void periodic() {
        ...
        // If we're running a PathfindingCommand locally, execute its periodic
        // lifecycle here so it can update outputs without being scheduled on
        // the global CommandScheduler and interfering with other bots.
        if (useLocalCommandRunner && activeCommand != null && activeCommandInitialized) {
    ...
    private void runSeekFuel(Pose2d pose) {
        ...
        for (Translation3d f : fuels) {
            ...
            // Preferred side check (kept inline to avoid unused variable warning)
            // if (sotm && ((ally.equals(Alliance.Red) && f.getX() > 12.2) || (ally.equals(Alliance.Blue) && f.getX() < 3.3))) {
            if (sotm) {
                if (d < bestPriority) {
                    bestPriority = d;
        ...
        if(sotm){
            if((ally.equals(Alliance.Red) && pose.getX() > 13.2) ||
               (ally.equals(Alliance.Blue) && pose.getX() < 3.3)) {
                if (fuelsCarried > 0 && shotTimer.get() > (1 / bps)) {
                    FuelSim.getInstance().shootFuelIntoHub(ally);
                    shotTimer.restart();
                    fuelsCarried--;
                }
    ...
    private void schedule(Pose2d target) {
        ...
        activeTarget = target;
        try {
            activeCommand = new IsolatedPathfindingCommand(
                    botPathfinder,
                    target,
    ...
    public static final Pose2d[] ROBOT_QUEENING_POSITIONS = new Pose2d[] {
        ...
        new Pose2d(5,  2, new Rotation2d()),
        new Pose2d(FieldConstants.fieldLength - 12.2, 0.6444996, new Rotation2d()),
